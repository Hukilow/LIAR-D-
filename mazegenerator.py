...
class Maze:
    ...
    def __init__(self, size_x, size_y):
        ...
        self.wall_size = np.array([size_y, size_x], dtype=np.int_)
        ...
        self.block_size = np.array([size_y * 2 + 1, size_x * 2 + 1], dtype=np.int_)
    ...
    def gen_maze_walls(self, corridor_len=999):
        ...
        # Then it will continue from a random point_ within the current maze (ie. visited cells), creating a junction, until no valid starting point_s exist.
        ...
        cell = np.array([random.randrange(2, self.wall_size[0]), random.randrange(2, self.wall_size[1])], dtype=np.int_)
        self.walls[cell[0], cell[1], 0] = 0

        # a simple definition of the four neighboring cells relative to current cell
        up    = np.array([-1,  0], dtype=np.int_)
        down  = np.array([ 1,  0], dtype=np.int_)
        left  = np.array([ 0, -1], dtype=np.int_)
        right = np.array([ 0,  1], dtype=np.int_)
        ...
        while np.size(cell) > 0 and self.running:
            ...
            if np.size(valid_neighbors) > 0:
                ...
                if round_nr - corridor_start < corridor_len:
                    ...
                    cell = np.array([neighbor[0], neighbor[1]], dtype=np.int_)
                else:
    ...
    def draw_cell(self, cell, neighbor):
        ...
        min_coord = (np.flip(np.minimum(cell, neighbor) * 2 - 1) * self.screen_block_size + self.screen_block_offset).astype(np.int_)
        max_coord = (np.flip(np.maximum(cell, neighbor) * 2 - 1) * self.screen_block_size + int(self.screen_block_size) + self.screen_block_offset).astype(np.int_)
...
if __name__ == '__main__':
    ...
    while running:
        ...
        # int_ialize a maze, given size (y, x)
